...
class AddRpush < ActiveRecord::Migration[5.0]
  ...
  class CreateRapnsNotifications < ActiveRecord::Migration[5.0]
    def self.up
      ...
      add_index :rapns_notifications,
                %i[delivered failed deliver_after],
    ...
    def self.down
      if index_name_exists?(
           :rapns_notifications,
           "index_rapns_notifications_multi"
         )
  ...
  class CreateRapnsFeedback < ActiveRecord::Migration[5.0]
    ...
    def self.down
      if index_name_exists?(
           :rapns_feedback,
           :index_rapns_feedback_on_device_token
         )
        remove_index :rapns_feedback,
                     name: :index_rapns_feedback_on_device_token
      end
  ...
  class AddAlertIsJsonToRapnsNotifications < ActiveRecord::Migration[5.0]
    def self.up
      add_column :rapns_notifications,
                 :alert_is_json,
                 :boolean,
                 null: true,
                 default: false
  ...
  class AddGcm < ActiveRecord::Migration[5.0]
    ...
    def self.up
      ...
      change_column :rapns_notifications,
                    :device_token,
                    :string,
                    null: true,
                    limit: 64
      ...
      reg_ids_type =
        (
          if ActiveRecord::Base.connection.adapter_name.include?("Mysql")
            :mediumtext
          else
            :text
          end
      ...
      AddGcm::Rapns::App.all.each do |app|
        AddGcm::Rapns::Notification.where(app: app.name).update_all(
          app_id: app.id
        )
      end
      ...
      if index_name_exists?(
        ...
        remove_index :rapns_notifications,
            ...
            :rapns_notifications,
            "index_rapns_notifications_on_delivered_failed_deliver_after"
          )
        remove_index :rapns_notifications,
                     name:
                       "index_rapns_notifications_on_delivered_failed_deliver_after"
      ...
      add_index :rapns_notifications,
                %i[app_id delivered failed deliver_after],
                name: "index_rapns_notifications_multi"
    end

    def self.down
      AddGcm::Rapns::Notification.where(
        type: "Rapns::Gcm::Notification"
      ).delete_all
      ...
      AddGcm::Rapns::App.all.each do |app|
        AddGcm::Rapns::Notification.where(app_id: app.id).update_all(
          app: app.key
        )
      end
  ...
  class RenameRapnsToRpush < ActiveRecord::Migration[5.0]
    ...
    def self.up
      ...
      if index_name_exists?(
        ...
        rename_index :rpush_notifications,
                     :index_rapns_notifications_multi,
                     :index_rpush_notifications_multi
      end
      ...
      if index_name_exists?(
        ...
        rename_index :rpush_feedback,
                     :index_rapns_feedback_on_device_token,
                     :index_rpush_feedback_on_device_token
      end

      update_type(
        RenameRapnsToRpush::Rpush::Notification,
        "Rapns::Apns::Notification",
      ...
      update_type(
        RenameRapnsToRpush::Rpush::Notification,
        "Rapns::Wpns::Notification",
        "Rpush::Wpns::Notification"
      )

      update_type(
        RenameRapnsToRpush::Rpush::App,
        "Rapns::Apns::App",
        "Rpush::Apns::App"
      )
      update_type(
