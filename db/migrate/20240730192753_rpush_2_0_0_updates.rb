class Rpush200Updates < ActiveRecord::Migration[5.0]
  ...
  def self.up
    add_column :rpush_notifications,
               :processing,
               :boolean,
               null: false,
               default: false
    ...
    if postgresql?
      execute(
        "ALTER TABLE rpush_feedback ALTER COLUMN app_id TYPE integer USING (trim(app_id)::integer)"
      )
    else
    ...
    %i[Apns Gcm Wpns Adm].each do |service|
      update_type(
        Rpush200Updates::Rpush::App,
        "Rpush::#{service}::App",
        "Rpush::Client::ActiveRecord::#{service}::App"
      )
      update_type(
        Rpush200Updates::Rpush::Notification,
  ...
  def self.adapter_name
    ...
    if ActiveRecord::VERSION::MAJOR > 6
      ActiveRecord::Base
        .configurations
        .configs_for(env_name: env)
        .first
        .configuration_hash[
        :adapter
      ]
    else
      Hash[ActiveRecord::Base.configurations[env].map { |k, v| [k.to_sym, v] }][
        :adapter
      ]
    end
  end
